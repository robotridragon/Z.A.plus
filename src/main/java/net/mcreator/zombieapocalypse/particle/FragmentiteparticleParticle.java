...
public class FragmentiteparticleParticle extends ZombieApocalypseModElements.ModElement {
	public static final BasicParticleType particle = new BasicParticleType(false);
	public FragmentiteparticleParticle(ZombieApocalypseModElements instance) {
		super(instance, 47);
	...
	public void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("fragmentiteparticle"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		protected CustomParticle(ClientWorld world, double x, double y, double z, double vx, double vy, double vz, IAnimatedSprite spriteSet) {
			...
			this.maxAge = (int) Math.max(1, 42 + (this.rand.nextInt(4) - 2));
		...
		public void tick() {
			super.tick();
			if (!this.isExpired) {
				this.setSprite(this.spriteSet.get((this.age / 7) % 6 + 1, 6));
			}
		}
	}
